...
def get_prices (inames) :
 return get_prices_account(main_account_id,inames)
...
# def close_all_account (account_id, iname) :
#  params = {"longUnits" : "ALL"}
#  response = oanda("PUT", "/v3/accounts/" + account_id + "/positions/EUR_USD/close", params);
#  return response
...
def trading1(nieme):
	...
	for i in range(1,2): # 1,nc
		...
		# summary = get_summary()
		details = get_details_account(main_account_id)
		long_details = get_details_account(long_account_id)
		short_details = get_details_account(short_account_id)
		...
		for j in range(3,dercol+1) :
			...
			elif semblable(sob,"NAV") :
				feuille.getCellByPosition(j,ligne).Value = get_nav()
			...
			elif len(sob) > 0 and sob[0] == '+' and sob[1:] in long_details :
				feuille.getCellByPosition(j,ligne).Value = float(long_details[sob[1:]])
				
			elif len(sob) > 0 and sob[0] == '-' and sob[1:] in short_details :
				feuille.getCellByPosition(j,ligne).Value = float(short_details[sob[1:]])
