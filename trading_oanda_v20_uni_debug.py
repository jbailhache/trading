...
def message(s) :
 global messages 
 messages = s + " / " + messages
 feuille.getCellRangeByName("D2").String = messages
...
def semblable(s1,s2) :
 # feuille.getCellRangeByName("F2").String = str(type(s1))
 # feuille.getCellRangeByName("G2").String = str(type(''))
 if s1 == s2 :
  # feuille.getCellRangeByName("E2").String = "egal"
  return True
 if type(s1) != type('') and type(s1) != type(u'') :
  # feuille.getCellRangeByName("E2").String = "1 non chaine"
  return False
 if type(s2) != type('') and type(s2) != type(u'') :
  # feuille.getCellRangeByName("E2").String = "2 non chaine"
  return False
 # feuille.getCellRangeByName("E2").String = "compare"
 return s1.strip().upper() == s2.strip().upper()

"""
hedg = feuille.getCellRangeByName("B28").String
#if True:
# feuille.getCellRangeByName("B29").String = hedg
hedging = None
...
if hedging :
 feuille.getCellRangeByName("D28").String = "oui"
 ...
 feuille.getCellRangeByName("D28").String = "non"
 account_id = feuille.getCellRangeByName("O4").String
 main_account_id = account_id
 # long_account_id = None
 # short_account_id = None
"""
...
def get_pip_location (iname) :
 global instruments
 feuille.getCellRangeByName("B20").String = "instr:("+str(instruments)+")"
 for instrument in instruments :
...
def str_order(o) :
	#try :
		...
		if not ("units" in o) :
			av = "A/V"
			cond = " " + o["type"] + " "
			s_order = " %6s : %-7s %5s %5s %-10s %2s %7s " % (o["id"], o["type"], av, "?u", "?i", cond, o["price"])
			return s_order
		elif int(o["units"]) > 0 :
			av = "Achat"
			if o["type"] == "LIMIT" :
...
def init_hedging () :
	...
	if hedging :
	 feuille.getCellRangeByName("D29").String = "oui"
	 long_account_id = feuille.getCellRangeByName("O3").String
	 short_account_id = feuille.getCellRangeByName("O4").String
	 main_account_id = long_account_id
	else :
	 feuille.getCellRangeByName("D29").String = "non"
	 account_id = feuille.getCellRangeByName("O4").String
	 main_account_id = account_id

	# feuille.getCellRangeByName("B29").String = account_id
	# feuille.getCellRangeByName("B30").String = main_account_id
	# feuille.getCellRangeByName("B31").String = long_account_id
	# feuille.getCellRangeByName("B32").String = short_account_id 


def trading1(nieme):

	global prev_orders, hedging, long_account_id, short_account_id, main_account_id, account_id
	...
	# feuille.getCellRangeByName("B42").String = "short:("+short_account_id+")"
		...
		for j in range(3,dercol+1) :
			...
			# if sob == "MARKET" or sob == "LIMIT" or sob == "STOP" or sob == "MARKET_IF_TOUCHED" :
				...
				if id1 != "" :
					...
					if un != 0 :
						if reel == 'oui' :
							# feuille.getCellByPosition(3,ligne).String = 'o2'
...
def tradingloop() :
	global account_id, key, instrs, instruments, prev_orders
	message("tradingloop")
	feuille = oDoc.CurrentController.ActiveSheet
